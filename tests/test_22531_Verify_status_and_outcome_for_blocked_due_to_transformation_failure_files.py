...
# 'Encrypted_spark.zip'
# ("\\grey4\Share\Sasha\_SASHA\TESTS\Automation\Portal\Test_files\Encrypted_spark.zip")

@pytest.mark.usefixtures("setup")
class TestOne:
    # Verifying initial status text
    def test_22531_verifying_initial_status_text(self):
        portal_link = actions_os.sanitize_file_via_dw(self, General.TESTFILE_CDR_FAILURE)
    ...
    def test_22531_verifying_status(self):
        ...
        status = file_portal_main_page.get_status_text()
        assert "Transformation failed" in status.text, '"Transformation failed" message is not displayed'

    # Verifying the outcome
    def test_22531_verifying_outcome(self):
        file_portal_main_page = FilePortalMainPage(self.driver)
        outcome = file_portal_main_page.get_outcome_text()
        assert Outcomes.OUTCOME_160 in outcome.text, Outcomes.OUTCOME_160 + ' message is not displayed'
