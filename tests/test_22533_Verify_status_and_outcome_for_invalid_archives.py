...
# 'Encrypted_damagedArch.zip'
# ("\\grey4\Share\Sasha\_SASHA\TESTS\Automation\Portal\Test_files\Encrypted_damagedArch.zip")

@pytest.mark.usefixtures("setup")
class TestOne:
    # Verifying initial status text
    def test_22533_verifying_initial_status_text(self):
        portal_link = actions_os.sanitize_file_via_dw(self, General.TESTFILE_INVALID_ARCHIVE)
    ...
    def test_22533_verifying_status(self):
        ...
        status = file_portal_main_page.get_status_text()
        assert "Invalid archive" in status.text, '"Invalid archive" message is not displayed'

    # Verifying the outcome
    def test_22533_verifying_outcome(self):
        file_portal_main_page = FilePortalMainPage(self.driver)
        outcome = file_portal_main_page.get_outcome_text()
        assert Outcomes.OUTCOME_142 in outcome.text, Outcomes.OUTCOME_142 + ' message is not displayed'
